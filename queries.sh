...
echo "$($PSQL "SELECT DISTINCT(name) FROM teams
                FULL JOIN games AS win ON teams.team_id = win.winner_id
                FULL JOIN games AS opp ON teams.team_id = opp.opponent_id
		WHERE (win.year=2014 AND win.round='Eighth-Final') OR (opp.year=2014 AND opp.round='Eighth-Final') ORDER BY name 
		")" 
